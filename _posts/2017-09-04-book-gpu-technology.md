...
本章では OpenGL のレンダリングパイプライン[^OpenGL-pipeline] (頂点シェーダ、テッセレーション、ラスタライズ、フラグメントシェーダなど) の各ステージでどのような処理をしているのか、また Intel HD Graphics において各ステージがどのように実装されているか分かりやすく解説されている。この他、ゲームにおける建物やキャラクターの影生成の話が面白かった。
...
前章がグラフィックス処理を中心にした解説だったのに対し、本章ではハードウェアを中心とした解説がされている。読んでいて一番面白い章だった。特にユニファイドメモリやメモリシステムの解説がとてもわかりやすくて良かった。一方、ワープスケジューラの挙動は一度読んだだけじゃよく分からなかった。
...
ニューラルネットワークの仕組み、画像認識 CNN、ASIC / FPGA、ディープラーニングでの GPU の活用、自動運転・3D モデリング・人の支援での活用、GPU 仮想化、スパコンのアクセラレータとしての利用、など
