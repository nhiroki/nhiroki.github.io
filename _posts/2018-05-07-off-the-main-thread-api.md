...
- **記法**: Web Worker はページとは別のファイルに書くか、Blob としてロードする必要がある。インラインで Web Worker を定義・実行する記法や、実行コンテキスト非依存な関数を定義してそれを共有できると良い？ (see: [Clooney](https://github.com/GoogleChromeLabs/clooney), [JS blocks](https://github.com/domenic/proposal-blocks))。既にあるパーツ (Transferables, Promises, Streams, ES Modules など) を使って自然な形でスレッド処理を統合できると良さそう？
...
- スレッド間・タスク間でのデータのやりとりはどうするのか？共有メモリ？メッセージパッシング？データの所有権モデル (コピーやムーブなど) はどうあるべきか？
- 実行するコードのやりとりはどうするのか？
